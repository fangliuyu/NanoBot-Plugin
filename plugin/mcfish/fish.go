...
func init() {
	engine.OnMessageRegex(`^进行(([1-4]?\d|50)次)?钓鱼$`, getdb).SetBlock(true).Limit(ctxext.LimitByUser).Handle(func(ctx *nano.Ctx) {
		...
		if numberOfPole > 30 {
			ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("你有", numberOfPole, "支鱼竿,大于30支的玩家不允许钓鱼"))
			return
		}
		fishNumber := 1
		...
		if info != "" {
			...
			if err != nil || number > FishLimit {
				ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("请输入正确的次数"))
				return
			}
			fishNumber = number
		}
		...
		if equipInfo == (equip{}) {
			...
			if !ok {
				ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("请装备鱼竿后钓鱼", err))
				return
			}
			ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("你尚未装备鱼竿,是否花费100购买鱼竿?\n回答\"是\"或\"否\""))
			// 等待用户下一步选择
			recv, cancel := nano.NewFutureEvent("Message", 999, false, nano.RegexRule(`^(是|否)$`), ctx.CheckSession()).Repeat()
			defer cancel()
			buy := false
			for {
				select {
				...
				case e := <-recv:
					...
					if money < 100 {
						ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("你钱包当前只有", money, "ATRI币,无法完成支付"))
						return
					}
		...
		if residue == 0 {
			ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("今天你已经进行", FishLimit, "次钓鱼了.\n游戏虽好,但请不要沉迷。"))
			return
		}
		fishNumber = residue
		msg := ""
		if equipInfo.Equip != "美西螈" {
			...
			if len(fishNmaes) == 0 {
				...
				ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("美西螈因为没吃到鱼,钓鱼时一直没回来,你失去了美西螈"))
				return
			}
			msg = "(美西螈吃掉了"
			fishNumber = 0
		...
		ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("你开始去钓鱼了,请耐心等待鱼上钩(预计要", time.Second*time.Duration(waitTime), ")"))
		...
		if len(thingNameList) == 1 {
			...
			if picName != "" {
				...
				if err != nil {
					...
					ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("恭喜你钓到了", numberOfFish, thingName, "\n", msg))
					return
				}
				ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("恭喜你钓到了", numberOfFish, thingName, "\n", msg), nano.ImageBytes(pic))
				return
			}
			ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("恭喜你钓到了", numberOfFish, thingName, "\n", msg))
			return
		}
		msgInfo := make([]string, 0, len(thingNameList))
		for name, number := range thingNameList {
			msgInfo = append(msgInfo, name+" : "+strconv.Itoa(number)+"\n")
		}
		ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("你进行了", fishNumber, "次钓鱼,结果如下:\n", strings.Join(msgInfo, "\n")))
	})
}
