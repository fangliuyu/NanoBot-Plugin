...
func init() {
	engine.OnMessageFullMatch("钓鱼背包", getdb).SetBlock(true).Limit(ctxext.LimitByUser).Handle(func(ctx *nano.Ctx) {
	...
	engine.OnMessageRegex(`^消除(绑定|宝藏)诅咒(\d*)$`, getdb).SetBlock(true).Limit(ctxext.LimitByUser).Handle(func(ctx *nano.Ctx) {
		uid := int64(ctx.UserID())
		...
		if number1 == 0 {
			ctx.SendChain(nano.Text("你没有绑定任何诅咒"))
			return
		}
		if number1 < number {
			number = number1
		...
		if number2 < number {
			ctx.SendChain(nano.Text("你没有足够的解除诅咒的道具"))
			return
		}
		...
		if err != nil {
			ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("消除失败,净化书销毁了\n[ERROR ]:", err))
			return
		}
		articles[0].Number -= number
		err = dbdata.updateUserThingInfo(uid, articles[0])
		...
		ctx.SendChain(nano.Text("消除成功"))
	})
	engine.OnMessageFullMatch("当前装备概率明细", getdb).SetBlock(true).Limit(ctxext.LimitByUser).Handle(func(ctx *nano.Ctx) {
		uid := int64(ctx.UserID())
		equipInfo, err := dbdata.getUserEquip(uid)
		if err != nil {
			ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("[ERROR]:", err))
		...
		msg := make(nano.Messages, 0, 20+len(thingList))
		...
		msg = append(msg, nano.Text("宝藏 : ", probableList[0], "%\n"))
		probable += probableList[1]
		msg = append(msg, nano.Text("鱼竿 : ", probableList[1], "%\n"))
		probable += probableList[2]
		msg = append(msg, nano.Text("鱼类 : ", probableList[2], "%\n"))
		probable += probableList[3]
		msg = append(msg, nano.Text("垃圾 : ", probableList[3], "%\n"))
		msg = append(msg, nano.Text("合计 : ", probable, "%\n"))
		msg = append(msg, nano.Text("-----------\n宝藏概率:\n"))
		for _, name := range treasureList {
			msg = append(msg, nano.Text(name, " : ",
