...
var (
	...
	refresh      = false
	timeNow      = 0
	refreshFish  = func(ctx *nano.Ctx) bool {
...
func limitSet(ctx *nano.Ctx) *rate.Limiter {
	return storeLimiter.Load(int64(ctx.UserID()))
}

func init() {
	engine.OnMessageFullMatchGroup([]string{"钓鱼看板", "钓鱼商店"}, getdb, refreshFish).SetBlock(true).Limit(limitSet).Handle(func(ctx *nano.Ctx) {
		...
		ctx.SendChain(nano.ImageBytes(pic))
	})
	engine.OnMessageRegex(`^出售(`+strings.Join(thingList, "|")+`)\s*(\d*)$`, getdb, refreshFish).SetBlock(true).Limit(limitSet).Handle(func(ctx *nano.Ctx) {
		...
		if strings.Contains(thingName, "竿") {
			...
			if times <= 0 {
				ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("出售次数已达到上限,明天再来售卖吧"))
				return
			}
		}
		...
		if len(articles) == 0 {
			ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("你的背包不存在该物品"))
			return
		}
		index := 0
		thing := article{}
		if len(articles) > 1 {
			...
			msg = append(msg, nano.ReplyTo(ctx.Message.ID), nano.ReplyTo(ctx.Message.ID), nano.Text("找到以下物品:\n"))
			for i, info := range articles {
				if info.Other != "" && info.Name != "美西螈" {
					msg = append(msg, nano.ReplyTo(ctx.Message.ID), nano.Text("[", i, "] ", info.Name, "(", info.Other, ")\n"))
				} else {
					msg = append(msg, nano.ReplyTo(ctx.Message.ID), nano.Text(
			...
			for {
				...
				case <-time.After(time.Second * 120):
					ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("等待超时,取消出售"))
					return
				case e := <-recv:
					nextcmd := e.Message.Content
					if nextcmd == "取消" {
						ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("已取消出售"))
						return
					}
		...
		pice /= 10
		ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("是否接受商店将以", pice*number*8/10, "收购", number, "个", thingName, "?\n回答\"是\"或\"否\""))
		// 等待用户下一步选择
		recv, cancel1 := nano.NewFutureEvent("Message", 999, false, nano.RegexRule(`^(是|否)$`), ctx.CheckSession()).Repeat()
		defer cancel1()
		buy := false
		for {
			select {
			case <-time.After(time.Second * 60):
				ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("等待超时,取消钓鱼"))
				return
			case e := <-recv:
				nextcmd := e.Message.Content
				if nextcmd == "否" {
		...
		if len(records) != 0 {
			...
			if numberOfRecord > 0 {
				ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("是否使用唱片让价格翻倍?\n回答\"是\"或\"否\""))
				// 等待用户下一步选择
				recv, cancel2 := nano.NewFutureEvent("Message", 999, false, nano.RegexRule(`^(是|否)$`), ctx.CheckSession()).Repeat()
				defer cancel2()
				use := false
				checkTime := false
				for {
		...
		if strings.Contains(thing.Name, "竿") || thing.Name == "三叉戟" {
			...
			if err1 != nil {
				ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("[ERROR at store.go.8]:", err1))
				return
			}
			if len(things) == 0 {
		...
		ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("出售成功,你赚到了", pice*number, msg))
	})
	engine.OnMessageRegex(`^购买(`+strings.Join(thingList, "|")+`)\s*(\d*)$`, getdb, refreshFish).SetBlock(true).Limit(limitSet).Handle(func(ctx *nano.Ctx) {
		uid := int64(ctx.UserID())
		...
		if numberOfPole > 50 {
			ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("你有", numberOfPole, "支鱼竿,大于50支的玩家不允许购买东西"))
			return
		}
		...
		if buytimes <= 0 {
			ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("购买次数已达到上限,明天再来购买吧"))
			return
		}
		thingName := ctx.State["regex_matched"].([]string)[1]
		number, _ := strconv.Atoi(ctx.State["regex_matched"].([]string)[2])
		...
		if len(thingInfos) == 0 {
			ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("当前商店并没有上架该物品"))
			return
		}
		if thingName == "净化书" {
			curse, err := dbdata.getNumberFor(uid, "宝藏诅咒")
			...
			if curse == 0 {
				ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.ReplyTo(ctx.Message.ID), nano.Text("你身上没有诅咒,不能购买该商品"))
				return
			}
			...
			if bless >= curse {
				ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.ReplyTo(ctx.Message.ID), nano.Text("你身上净化书已足够,不能购买该商品"))
				return
			}
			if curse < number {
				number = curse
		...
		if len(thingInfos) > 1 {
			msg := make(nano.Messages, 0, 3+len(thingInfos))
			msg = append(msg, nano.ReplyTo(ctx.Message.ID), nano.Text("找到以下物品:\n"))
			...
			for {
				select {
				case <-time.After(time.Second * 120):
					ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.ReplyTo(ctx.Message.ID), nano.Text("等待超时,取消购买"))
					return
				case e := <-recv:
					nextcmd := e.Message.Content
					if nextcmd == "取消" {
						ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("已取消购买"))
						return
					}
					index, err = strconv.Atoi(nextcmd)
		...
		if thing.Number < number {
			ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("商店数量不足"))
			return
		}
		...
		if money < price {
			ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("你身上的钱(", money, ")不够支付", msg))
			return
		}

		ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("你确定花费", price, "购买", number, "个", thingName, "?", msg, "\n回答\"是\"或\"否\""))
		// 等待用户下一步选择
		recv, cancel1 := nano.NewFutureEvent("Message", 999, false, nano.RegexRule(`^(是|否)$`), ctx.CheckSession()).Repeat()
		defer cancel1()
		buy := false
		for {
			select {
			case <-time.After(time.Second * 60):
				ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("等待超时,取消购买"))
				return
			case e := <-recv:
				nextcmd := e.Message.Content
				if nextcmd == "否" {
		...
		if !ok {
			ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.ReplyTo(ctx.Message.ID), nano.Text("你慢了一步,物品被别人买走了"))
			return
		}
		thing.Number -= number
		...
		if strings.Contains(thingName, "竿") || thingName == "三叉戟" {
			...
			if err1 != nil {
				ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("[ERROR at store.go.15]:", err1))
				return
			}
			if len(things) == 0 {
		...
		ctx.SendChain(nano.ReplyTo(ctx.Message.ID), nano.Text("你用", price, "购买了", number, thingName))
	})
}
...
func drawStroeInfoImage(stroeInfo []store) (picImage image.Image, err error) {
	...
	canvas.SetColor(color.Black)
	textDy += textDh * 2
	canvas.DrawStringAnchored("注:出售商品 = 价格*10% * (1-20%)", 10, textDy+textDh/2, 0, 0.5)
